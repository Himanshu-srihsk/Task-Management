...
export const fetchSubmissionsByTaskId = createAsyncThunk("submissions/fetchSubmissionsByTaskId",
    async(taskId) => {
        setAuthHeader(localStorage.getItem("jwt"),api)
        try {
            console.log("Submission list api fectch");
            const {data} =await api.get(`/api/submissions/task/${taskId}`)
            console.log("fetchSubmissionsByTaskId success", data);
            return data;
        } catch (error) {
            console.error("Error fetchSubmissionsByTaskId ", error);
            throw Error(error.response.data.error);
        }
    }
)
...
export const acceptDeclineSubmission = createAsyncThunk("submissions/acceptDeclineSubmission",
    async({id, status}) => {
        ...
        try {
            ...
            console.log("accept Submission success", data);
            return data;
        } catch (error) {
            console.error("Error acceptDeclineSubmission ", error);
            throw Error(error.response.data.error);
        }
    }
)
